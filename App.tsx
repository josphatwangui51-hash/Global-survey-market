...
export default function App() {
  ...
  useEffect(() => {
    ...
    const setupNotifications = async () => {
      ...
      // If granted and user is logged in, start the interval
      if (Notification.permission === 'granted' && currentUser) {
        interval = setInterval(() => {
          // 40% chance of triggering a notification every 45 seconds to simulate activity
          if (Math.random() > 0.6) {
             const rewards = [80, 150, 250, 400];
             const reward = rewards[Math.floor(Math.random() * rewards.length)];
             
             try {
                // Create notification
                const notif = new Notification("New Premium Survey Match!", {
                    body: `A new high-value survey matches your profile. Earn KES ${reward} now!`,
                    tag: 'survey-alert', // Use tag to prevent stacking too many
                    renotify: true
                } as any);
